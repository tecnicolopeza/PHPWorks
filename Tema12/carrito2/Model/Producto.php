...
class Producto
{
    ...
    public static function searchProductByPrice($minPrice,$maxPrice){
        $conexion = CarritoDB::connectDB();
        $selection = "SELECT * FROM products WHERE price >= ".$minPrice." AND price <= ".$maxPrice." ORDER BY price ASC";
        $consult = $conexion->query($selection);
        $products = [];
        while ($registro = $consult->fetchObject()) {
            $products[] = new Producto(
