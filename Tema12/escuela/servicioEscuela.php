...
if ($metodo == 'GET') {
  if (isset($_REQUEST['cursos'])) {
    $resultado = Alumno::getAlumnosByCurso($_REQUEST['cursos']);
    foreach ($resultado as $fila) {
      $devolver['alumnos'][] = ['matricula' => $fila->getMatricula(), 'nombre' => $fila->getNombre(), 'apellidos' => $fila->getApellidos(), 'curso' => $fila->getCurso()];
    }
  } elseif (isset($_REQUEST['nombreAlumno'])) {
    $resultado = Alumno::getAlumnosByName($_REQUEST['nombreAlumno']);
    foreach ($resultado as $fila) {
      $devolver['alumnos'][] = ['matricula' => $fila->getMatricula(), 'nombre' => $fila->getNombre(), 'apellidos' => $fila->getApellidos(), 'curso' => $fila->getCurso()];
    }
  } elseif (isset($_REQUEST['matricula'])) {
    $resultado = AlumnoAsignatura::getAsignaturaByMatricula($_REQUEST['matricula']);
    foreach ($resultado as $fila) {
      $devolver['asignaturas'][] = ['nombre' => $fila->getNombre()];
    }
  }
