...
class Producto
{
    ...
    public static function getProducts($order=NULL)
    {
        $conexion = CarritoDB::connectDB();
        $selection = "SELECT * FROM products";

        if($order=="a"){
            $selection .= " ORDER BY name ASC";
        }elseif($order=="d"){
            $selection .= " ORDER BY name DESC";
        }elseif($order=="c"){
            $selection .= " ORDER BY price ASC";
        }elseif($order=="h"){
            $selection .= " ORDER BY price DESC";
        }
